...
// ✅ Fetch trainers and admins
$staffResult = $conn->query("SELECT id, firstName, middleName, lastName, email, username, role, status, created_at FROM users WHERE role IN ($placeholders) ORDER BY created_at DESC");
$staff = [];
while ($row = $staffResult->fetch_assoc()) {
  // Combine full name
  $row['full_name'] = trim($row['firstName'] . ' ' . ($row['middleName'] ?? '') . ' ' . $row['lastName']);
  $staff[] = $row;
}
...
function getTodaySessions($appointments, $trainerId)
{
  ...
  foreach ($appointments as $app) {
    ...
    if ($app['date'] === $today) {
      $appTime = strtotime($app['date'] . " " . $app['start_time']);
      $now = time();
...
<head>
  ...
  <style>
    ...
    #sidebar {
      position: fixed; top: 56px; left: 0; width: 250px; bottom: 0;
      background: #fff; border-right: 1px solid #dee2e6;
      overflow-y: auto; padding-top: 20px;
    }
    main {
      margin-left: 250px; margin-top: 56px;
      height: calc(100vh - 56px); overflow-y: auto; padding: 20px;
    }
